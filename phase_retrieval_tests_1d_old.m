...
image_example=1; %Use 1,2,3 (see below)
...
opts.method='ap'; opts.mu1=1; opts.mu2=0.1;
...
for it=1:numit
    ...
    elseif (strcmp(opts.method,'ap'))
        ...
        fproj=alpha1*f.*(f>=0).*(abs(xx)<1)+(1-alpha1)*((f<0)|(abs(xx)>=1));
    ...
    if (strcmp(opts.method,'ap'))
        fhatmag=alpha2*u+(1-alpha2)*abs(new_fhat);
    end;
